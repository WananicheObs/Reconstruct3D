...
VkPipeline PipelineBuilder::build_pipeline(VkDevice device)
{
    ...
    std::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments(_renderInfo.colorAttachmentCount);
    for (int i = 0; i < colorBlendAttachments.size(); ++i) {
        colorBlendAttachments[i] = _colorBlendAttachment; // Copy the same state
    }
    ...
    colorBlending.attachmentCount = colorBlendAttachments.size();   //so that it goes with the count set at set_color_attachment_format
...
void PipelineBuilder::set_color_attachment_format(int count, VkFormat* format)
{
    /*
    ...
    */
    ...
    _renderInfo.pColorAttachmentFormats = format;
}
